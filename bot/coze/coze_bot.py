# access Coze knowledge base platform
...
class CozeBot(Bot):
    ...
    def __init__(self):
        super().__init__()
        self.sessions = CozeSessionManager(CozeSession, model=conf().get("model") or "gpt-4")
        self.args = {}
    ...
    def reply(self, query, context: Context = None) -> Reply:
        if context.type == ContextType.TEXT:
            ...
            if not conf().get("text_to_image"):
                logger.warn("[Coze] text_to_image is not enabled, ignore the IMAGE_CREATE request")
...
class CozeSessionManager(SessionManager):
...
class CozeSession(ChatGPTSession):
